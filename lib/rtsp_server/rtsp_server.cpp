...
bool rtsp_server::client_handler(void *arg)
{
	...
	// Actualiza métricas si hubo al menos un cliente
	if (!self->clients_.empty())
	{
	  if (self->last_frame_ms_ != 0)
	  {
	    uint32_t dt = now - self->last_frame_ms_;
	    if (dt > 0)
	    {
	      float inst_fps = 1000.0f / dt;
	      self->fps_ewma_ = (self->fps_ewma_ == 0.0f)
	                            ? inst_fps
	                            : (FPS_ALPHA_ * inst_fps + (1.0f - FPS_ALPHA_) * self->fps_ewma_);
	    }
	  }
